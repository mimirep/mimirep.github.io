...
​	토기는 고고학 연구에 있어서 가장 많이 활용되는 유물 카테고리 중 하나이다. 실생활 용기부터 제기에 이르기까지 사용되지 않는 곳이 없고, 많이 출토되며, 무엇보다 시간성 및 지역성이 뚜렷하여 우리에게 많은 정보를 준다. 이러한 토기의 기능적인 측면에서는 역시 용량(容量)이 가장 중요할 것이다. 토기의 용량을 통해 어떤 내용물이 담겼는지 혹은 어떤 용도로 사용되었는지를 일차적으로 추측해볼 수 있을 것이다.

​	본 글에서는 앞선 연구인 **김범철, 2007, 「무문토기의 용량분석 시론 - 충남지역 청동기시대 취락 출토품을 중심으로」, 『야외고고학』 2.** 를 살펴보고 해당 논문에서 사용된 단순선형회귀분석이 아닌 다중선형회귀를 통해 용량을 예측해본 뒤 두 결과값을 비교해보고자 한다.

<br>

### 2. 선행연구

**김범철, 2007, 「무문토기의 용량분석 시론 - 충남지역 청동기시대 취락 출토품을 중심으로」, 『야외고고학』 2.** 에서는 구경, 목지름, 견부각, 기고를 활용하여 단순선형회귀분석을 시행한 뒤 실용량을 예측하는 방법을 사용하였다.

​	하지만 이러한 방법의 경우, 다양한 변수에 의해 복합적인 영향을 받는 고고학 자료의 특성을 반영할 수 없다. 실제로 해당 논문에서 시행한 송국리형토기에 대한 분석의 경우 결정계수인 $$R^2$$ 값이 0.55로, 보통 신뢰의 기준이 되는 0.7을 하회한다. 또한 실용량과 수식으로 복원된 용량간의 차이가 제법 존재한다.

​	이러한 문제점을 해결하기 위해 다중선형회귀분석을 시행하여 분석해보고자 한다.
...
data = pd.read_csv("김범철_2009_Data.csv")
...
​	전체 코드를 Python으로 짰으며, Jupyter Notebook 환경에서 코딩을 진행하였다. Pandas, Matplotlib, Sklearn 라이브러리를 사용하였다. 
...
​	CSV 파일로 준비해둔 데이터를 불러온 뒤, X에는 구경, 목지름, 견부각, 기고에 대한 정보를 넣어주었고 Y에는 실용량에 대한 정보를 넣어주었다. 그 후 데이터를 학습데이터(X_train, Y_train)는 60%, 테스트데이터(X_test, Y_test)는 40%로 분할해주었다. 
...
​	Sklearn의 선형회귀모델을 가져온 뒤 fit을 사용하여 앞서 마련한 학습데이터를 학습시켜주었다.
...
$$
y = b + a_1x_1 + a_2x_2 + a_3x_3 + ... + a_nx_n
$$

​	다중선형회구분석의 수식은 위와 같다. coef를 사용하여 회귀계수에 해당하는 $$A_n$$ 을 산출했고, intercept를 사용하여 상수에 해당하는 b를 산출해주었다. 사용된 변수가 4개이기 때문에 회귀계수 또한 4개가 산출된 것을 확인할 수 있다.

​	그리고 score를 사용하여 결정계수 $$R^2$$ 를 구해서 모델의 정확도를 평가해주었다. 이 모델의 결정계수는 0.97로, 앞서 사용한 변수들을 활용하여 약 97%를 설명할 수 있는 것이다. 아래는 산출된 값들이다.
...
​	앞서 말했다시피 김범철 선생님께서는 견부각을 4개로 세분화하여 단순선형회귀분석을 진행하셨다. 하지만 그러기에는 데이터의 수가 너무 적기 때문에 아예 견부각을 제외하고 구경, 목지름, 기고만을 가지고 다중선형회귀분석을 시행했다. 

​	그 결과 정확도는 0.95로 조금 낮아졌으나 아직 유효한 수준의 정확도였고, 이 모델을 통해 예측한 용량값은 김범철 선생님의 수식을 사용하여 예측한 용량값보다는 실용량값에 더 근접하였다. 하지만 견부각을 포함한 다중선형회귀 모델과의 우열은 가리기 힘든 수준이다. 해당 표는 결과에서 소개하도록 하겠다.
